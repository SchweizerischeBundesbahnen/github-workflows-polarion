...
- CodeQL's `actions/untrusted-checkout` fires on the NAME of the checkout's `ref` expression, not on where the value comes from: under a `workflow_call` trigger any field name matching `.*(head|branch|ref).*` or `.*(head|sha|commit).*` is flagged, whatever it holds (`UntrustedCheckoutQuery.qll`, read 2026-08-22). So renaming an input clears the alert while the caller still chooses the checkout — take the caller's control away instead, as alert 1 was resolved in PR #100
